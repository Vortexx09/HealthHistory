  ...
  <div class="min-h-screen bg-gradient-to-br from-primary-50 to-secondary-50 px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
    <div class="mx-auto w-full max-w-md">
      ...
      <div class="mb-8 text-center">
        ...
        <h1 class="text-2xl font-bold text-secondary-900">Bienvenido de nuevo</h1>
        <p class="mt-2 text-secondary-600">Inicia sesión en tu cuenta de HealthHistory</p>
      </div>
        ...
        <form @submit.prevent="handleLogin" class="space-y-6">
          ...
          <div class="flex items-center justify-between">
            <label class="flex items-center gap-2">
              ...
              />
              <span class="text-sm text-secondary-600">Recordarme</span>
            </label>
      ...
      <!-- Caja de información (demo) -->
      <div class="mt-6 rounded-lg bg-blue-50 p-4 border border-blue-200">
        <p class="text-sm text-blue-900">
          <strong>Credenciales de demostración:</strong> Usa cualquier número de identificación y contraseña para probar el formulario (se requiere integración con el backend)
        </p>
      </div>
    </div>
  </div>
...
const handleLogin = async () => {
  try {
    ...
    setTimeout(() => {
      if (user?.value?.user_type === 'doctor') {
